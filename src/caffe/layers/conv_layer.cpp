...
// overwriting the Forward_fpga and Backward_fpga only for convolution
template <typename Dtype>
void ConvolutionLayer<Dtype>::Forward_fpga(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    ...
    for (int n = 0; n < this->num_; ++n) {
      ...
      if (this->bias_term_) {
        const Dtype* bias = this->blobs_[1]->cpu_data();
        this->forward_fpga_bias(top_data + n * this->top_dim_, bias);
      }
    }
  }
}
...
template <typename Dtype>
void ConvolutionLayer<Dtype>::Backward_fpga(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  const Dtype* weight = this->blobs_[0]->cpu_data();
  Dtype* weight_diff = this->blobs_[0]->mutable_cpu_diff();
  for (int i = 0; i < top.size(); ++i) {
    ...
    if (this->bias_term_ && this->param_propagate_down_[1]) {
      Dtype* bias_diff = this->blobs_[1]->mutable_cpu_diff();
      for (int n = 0; n < this->num_; ++n) {
        this->backward_fpga_bias(bias_diff, top_diff + n * this->top_dim_);
      }
    }
    if (this->param_propagate_down_[0] || propagate_down[i]) {
      for (int n = 0; n < this->num_; ++n) {
